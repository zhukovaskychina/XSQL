		...
		if(pbrother->GetCount() > idx_->rank()) {
			...
			if(pnode->GetIsLeaf()) {
				...
				if(pbrother->GetValues(pbrother->GetCount())>=0) {
					...
					GetNode(pbrother->GetValues(pbrother->GetCount()))->SetParent(pnode->block_num());
...
void BPlusTreeNode::SetParent(int val) {
    *((int*)(&buffer_[8]))=val;
}

void BPlusTreeNode::SetNodeType(int val) {
    *((int*)(&buffer_[0]))=val;
}

void BPlusTreeNode::SetCount(int val) {
	*((int*)(&buffer_[4]))=val;
}

void BPlusTreeNode::SetIsLeaf(bool val) {
	SetNodeType(val?1:0);
}


void BPlusTreeNode::GetBuffer() {
    BlockInfo *block = tree_->hdl()->GetFileBlock(tree_->db_name(), tree_->idx()->name(), FORMAT_INDEX, block_num_);
    buffer_ = block->data();
...
BPlusTreeNode* BPlusTreeNode::Split(TKey &key) {
	BPlusTreeNode* newnode = new BPlusTreeNode(true, tree_, tree_->GetNewBlockNum(), GetIsLeaf());
	if(newnode == NULL) {
		throw BPlusTreeException();
		return NULL;
	}
