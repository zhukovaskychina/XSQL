...
void RecordManager::Insert(InsertParser &insertParser) {
    string tb_name = insertParser.tableName;
    unsigned long values_size = insertParser.values.size();
    ...
    //此处用来实现colName和colValue对应
    tbl->getAttribute();
    std:vector<SQLValue> stValues=insertParser.values;
    int max_count = (4096-12)/(tbl->record_length());
    ...
    for(int i=0; i<values_size; ++i){
        int value_type = stValues[i].data_type;
        string value = stValues[i].value;
        int length = tbl->ats()[i].length();
