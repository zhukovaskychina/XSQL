...
void Database::CreateTable(CreateTableParser& createTableParser) {
    int record_length = 0;
    Table tb;
    std::vector<Attribute> attrs=createTableParser.getAttributes();
    for (int i=0; i<attrs.size(); ++i) {
        tb.AddAttribute(attrs[i]);
        record_length += attrs[i].length();
    }
    tb.set_tb_name(createTableParser.getTableName());
